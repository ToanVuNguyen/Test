...
-- Project : HPF 
-- Build 
-- Description:	Create new HPF database
--		Apply database changes on:  30 Dec 2008. 
...
CREATE TABLE funding_source (
  ...
  chg_lst_user_id VARCHAR(15)  NOT NULL  ,
...
CREATE TABLE Area_Median_Income (
  ...
  metro INT    ,
  create_dt DATETIME    ,
  create_user_id VARCHAR(15)    ,
  create_app_name VARCHAR(20)    ,
  chg_lst_dt DATETIME    ,
  chg_lst_user_id VARCHAR(15)    ,
...
CREATE INDEX IFK_invoice_Funding_source_FK ON Invoice (funding_source_id);
GO


CREATE TABLE agency_payable (
  agency_payable_id INTEGER  NOT NULL   IDENTITY ,
  Agency_agency_name VARCHAR(50)  NOT NULL  ,
...
PRIMARY KEY(agency_payable_id)  ,
  FOREIGN KEY(agency_id, Agency_agency_name)
    REFERENCES Agency(agency_id, agency_name));
GO


CREATE INDEX agency_payment_FKIndex1 ON agency_payable (agency_id, Agency_agency_name);
GO


CREATE INDEX IFK_agency_payable_Agency_FK ON agency_payable (agency_id, Agency_agency_name);
GO
...
CREATE INDEX IFK_ref_code_item_Ref_code_set ON ref_code_item (ref_code_set_name);
GO
...
CREATE INDEX IFK_payment_Funding_source_FK ON invoice_payment (funding_source_id);
GO
...
CREATE INDEX IFK_budget_subcategory_Budget_ ON budget_subcategory (budget_category_id);
GO
...
CREATE INDEX IFK_Rel_34 ON call (call_center_id);
GO
...
GO
CREATE INDEX ws_user_FKIndex2 ON ws_user (agency_id, Agency_agency_name);
GO


CREATE INDEX IFK_ws_user_Call_center_FK ON ws_user (call_center_id);
GO
CREATE INDEX IFK_ws_user_Agency_FK ON ws_user (agency_id, Agency_agency_name);
GO
...
CREATE INDEX agent_rate_FKIndex1 ON agency_rate (agency_id, Agency_agency_name);
GO
CREATE INDEX agency_rate_FKIndex2 ON agency_rate (program_id);
GO


CREATE INDEX IFK_agency_rate_Agency_FK ON agency_rate (agency_id, Agency_agency_name);
GO
CREATE INDEX IFK_agency_rate_Program_FK ON agency_rate (program_id);
GO
...
CREATE INDEX IFK_funding_source_rate_Fundin ON funding_source_rate (funding_source_id);
GO
CREATE INDEX IFK_FundingSrc_rate_Program_FK ON funding_source_rate (program_id);
GO
...
CREATE INDEX IFK_funding_source_group_Fundi ON funding_source_group (funding_source_id);
GO
CREATE INDEX IFK_funding_source_group_Servi ON funding_source_group (servicer_id);
GO
...
CREATE TABLE foreclosure_case (
  ...
  counselor_id_ref VARCHAR(15)  NOT NULL  ,
  ...
  fc_sale_date_set_ind VARCHAR(1)    ,
...
CREATE INDEX Counselling_session_FKIndex1 ON foreclosure_case (agency_id, Agency_agency_name);
GO
...
CREATE INDEX IFK_foreclosure_case_Agency_FK ON foreclosure_case (agency_id, Agency_agency_name);
GO
CREATE INDEX IFK_foreclosure_case_Call_FK ON foreclosure_case (call_id);
GO
CREATE INDEX IFK_foreclosure_case_Program_F ON foreclosure_case (program_id);
GO
...
CREATE INDEX IFK_activity_log_FCase_FK ON activity_log (fc_id);
GO
...
CREATE INDEX IFK_case_post_counseling_statu ON case_post_counseling_status (fc_id);
GO
...
CREATE INDEX IFK_budget_set_FCase_FK ON budget_set (fc_id);
GO
...
CREATE INDEX IFK_case_loan_Foreclosure_case ON case_loan (fc_id);
GO
...
CREATE INDEX IFK_foreclosure_case_audit_FCa ON case_audit (fc_id);
GO
...
CREATE INDEX IFK_agency_payment_case_Agency ON agency_payable_case (agency_payable_id);
GO
CREATE INDEX IFK_agency_payable_case_FCase_ ON agency_payable_case (fc_id);
GO
...
CREATE INDEX IFK_budget_item_Budget_set_FK ON budget_item (budget_set_id);
GO
CREATE INDEX IFK_budget_item_Budget_categor ON budget_item (budget_subcategory_id);
GO
...
CREATE INDEX IFK_outcome_item_Outcome_type_ ON outcome_item (outcome_type_id);
GO
CREATE INDEX IFK_Rel_34 ON outcome_item (fc_id);
GO
...
CREATE INDEX IFK_invoice_case_Invoice_FK ON invoice_case (Invoice_id);
GO
CREATE INDEX IFK_invoice_case_Payment_FK ON invoice_case (invoice_payment_id);
GO
CREATE INDEX IFK_invoice_case_FCase_FK ON invoice_case (fc_id);
GO
