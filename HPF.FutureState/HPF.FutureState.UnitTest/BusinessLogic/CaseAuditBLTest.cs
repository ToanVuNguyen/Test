...
namespace HPF.FutureState.UnitTest
{
    ...
    /// <summary>
    ///This is a test class for CaseAuditBLTest and is intended
    ///to contain all CaseAuditBLTest Unit Tests
    ///</summary>
    [TestClass()]
    public class CaseAuditBLTest
    {


        private TestContext testContextInstance;
        private static string working_user_id = "wuid_CaseAuditDAOTest_6868";
        private static string working_user_id_insert_test = "wuid_CADAOTEST_insert_6868";
        private static int fc_id = 0;
        private static int fc_id_insert = 0;
        ...
        {
            CaseAuditBL_Accessor target = new CaseAuditBL_Accessor(); // TODO: Initialize to an appropriate value            
            int expected = 1; // TODO: Initialize to an appropriate value
            CaseAuditDTOCollection collection = target.RetrieveCaseAudits(fc_id);
        ...
        public void InsertCaseAuditTest_Pass()
        {
            ...
            var actual = target.SaveCaseAudit(caseAudit, working_user_id, false);
            Assert.AreEqual(true, actual);
        }

        [TestMethod()]
        [ExpectedException(typeof(DataValidationException))]
        public void InsertCaseAuditTest_Fail()
        {
            CaseAuditBL_Accessor target = new CaseAuditBL_Accessor(); // TODO: Initialize to an appropriate value
            CaseAuditDTO caseAudit = new CaseAuditDTO()
            {
        ...
        public void UpdateCaseAuditTest_Pass()
        {
            ...
            var actual = target.SaveCaseAudit(caseAudit, working_user_id, true);
            Assert.AreEqual(true, actual);

            caseAudit = target.RetrieveCaseAudits(fc_id)[0];
            Assert.AreEqual(audit_comment, caseAudit.AuditComments);
        }
        
        [TestMethod()]
        [ExpectedException(typeof(DataValidationException))]
        public void UpdateCaseAuditTest_Fail()
        {
            CaseAuditBL_Accessor target = new CaseAuditBL_Accessor(); // TODO: Initialize to an appropriate value
            CaseAuditDTO caseAudit = new CaseAuditDTO()
            {
        ...
        static private void GenerateTestData()
        {
            ...
            sql = "Insert into foreclosure_case "
               ...
               + ", 'cfname', 'clname', 'cidref'"
               + ", '12345', 'acn121314', '1234'"
               + ", 'HPF' ,'" + working_user_id_insert_test + "' ,'" + DateTime.Now + "', 'HPF', '" + working_user_id_insert_test + "', '" + DateTime.Now + "' )";
            ExecuteSql(sql, dbConnection);
            #endregion
            fc_id_insert = GetFcID(working_user_id_insert_test);
            fc_id = GetFcID(working_user_id);
        ...
        static private void ClearTestData()
        {
            ...
            sql = "Delete from foreclosure_case where create_user_id = '" + working_user_id_insert_test + "'";
            ExecuteSql(sql, dbConnection);
        ...
        static private int GetFcID(string wui)
        {
            int result = 0;
            string sql = "SELECT fc_id FROM foreclosure_case " +
                            " WHERE create_user_id = '" + wui + "'";
