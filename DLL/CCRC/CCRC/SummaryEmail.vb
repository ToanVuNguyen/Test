...
Public Class ReferralEmailHelper


    Public Function CouselingSummaryEmailBody(ByVal referralId, ByVal loanId, ByVal zipCode, ByVal profileSeqId, ByVal sUserId) As String

        Dim borrowerName, agencyName, counselorName, servicerName, servicerContactName, servicerContactEmail
        Dim agencyTrackingId, referralType
        Dim referralDate, referralComment, approvedBy
        Dim lastName, firstName, miName
        Dim addrLine1, city, state
        ...
        Dim sDomainUser, sDomain, sSQL, iReturnCode, sMessage
        ...
        Rs = obj.Get_Referral(loanId, referralId, zipCode, profileSeqId, sUserId, sSQL, iReturnCode, sMessage)
        ...
        If isValidRS(Rs) Then
            ...
            EmailComments = Rs.Fields("EMAIL_NOTE").Value
        ...
        emailBody3 = emailBody3 & "<tr><td><b>Counselor Comments:</b> " & EmailComments & "</td></tr>" & vbCrLf

        emailBody3 = emailBody3 & "<tr><td height='10'></td></tr>" & vbCrLf
        emailBody3 = emailBody3 & "</table>" & vbCrLf

        Return emailHeader + vbCrLf + emailBody + vbCrLf + emailBody2 + vbCrLf + emailBody3

    End Function

    Private Function isValidRS(ByVal rsIn)
        isValidRS = False
    ...
    Private Function stripCharsInBag(ByVal s, ByVal bag)
    ...
    Public Function EmailLogWithAttachment(ByVal Emailto, ByVal Emailfrom, ByVal Referralseqid, ByVal Source, ByVal Attachment1, ByVal Attachment2, ByVal Attachment3) As String

        Dim Sqlstr As String

        Sqlstr = "INSERT INTO referral_email_summary ( sent_date, email_to, email_from, referral_seq_id, first_mortgage_servicer,first_mortgage_seq_id, loan_id, zip, last_name, referral_date, referral_result, referral_result_date, agency, agency_referral_id, counselor_first_name, counselor_last_name, acct_prd, attachment_1, attachment_2, attachment_3,Source) SELECT TOP (1) GETDATE() AS SENT_DATE, '" & Emailto & "' AS EMAIL_TO, '" & Emailfrom & "' AS EMAIL_FROM, Ref.REFERRAL_SEQ_ID, BSN_Servicer.BSN_ENTITY_NAME AS FIRST_MORTGAGE_SERVICER, bsn_servicer.BSN_ENTITY_SEQ_ID FIRST_MORTGAGE_SEQ_ID, Ref.LOAN_ID, Ref.ZIP, Ref.LAST_NAME, CONVERT(CHAR(10), Ref.REFERRAL_DATE, 101) AS REFERRAL_DATE, Ref_Result.DSCR AS REFERRAL_RESULT, CONVERT(CHAR(10), REF_RESULT_TYPE_HIST.REFERRAL_RESULT_DATE, 101) AS REFERRAL_RESULT_DATE, Agency.BSN_ENTITY_NAME AS AGENCY, Ref.AGENCY_REFERRAL_ID, CCRC_USER.FIRST_NAME AS COUNSELOR_FIRST_NAME, CCRC_USER.LAST_NAME AS COUNSELOR_LAST_NAME, REF_RESULT_TYPE_HIST.ACCT_PRD, '" & Attachment1 & "' AS ATTACHMENT_1, '" & Attachment2 & "' AS ATTACHMENT_2, '" & Attachment3 & "' AS ATTACHMENT_3,'" & Source & "' FROM BSN_ENTITY AS Agency INNER JOIN REFERRAL AS Ref LEFT OUTER JOIN REF_RESULT_TYPE_HIST AS REF_RESULT_TYPE_HIST LEFT OUTER JOIN REFERRAL_RESULT_TYPE AS Ref_Result ON REF_RESULT_TYPE_HIST.REFERRAL_RESULT_TYPE_CODE = Ref_Result.REFERRAL_RESULT_TYPE_CODE ON Ref.REFERRAL_SEQ_ID = REF_RESULT_TYPE_HIST.REFERRAL_SEQ_ID ON Agency.BSN_ENTITY_SEQ_ID = Ref.AGENCY_SEQ_ID INNER JOIN BSN_ENTITY AS BSN_Servicer ON Ref.SERVICER_SEQ_ID = BSN_Servicer.BSN_ENTITY_SEQ_ID INNER JOIN CCRC_USER AS CCRC_USER ON Ref.COUNSELOR_SEQ_ID = CCRC_USER.CCRC_USER_SEQ_ID WHERE (Ref.REFERRAL_SEQ_ID = '" & Referralseqid & "') ORDER BY REF_RESULT_TYPE_HIST.ACCT_PRD DESC, REFERRAL_RESULT_DATE DESC"
        ...
        Return "Email Logged."
    End Function
    Public Function UpdateEmailLog(ByVal REFERRALEMAILSEQID, ByVal Sentdate, ByVal Emailto, ByVal Emailfrom, ByVal Referralseqid, ByVal Firstmortgageservicer, ByVal Loanid, ByVal Zip, ByVal Lastname, ByVal Referraldate, ByVal Referralresult, ByVal Referralresultdate, ByVal Agency, ByVal Agencyreferralid, ByVal Counselorfirstname, ByVal Counselorlastname, ByVal Acctprd) As String

        Dim Sqlstr As String

        Sqlstr = "UPDATE [REFERRAL_EMAIL_SUMMARY] Set [SENT_DATE] ='" & Sentdate & "'" & ",[EMAIL_TO]  ='" & Emailto & "'" & ",[EMAIL_FROM] ='" & Emailfrom & "'" & ",[REFERRAL_SEQ_ID]  ='" & Referralseqid & "'" & ",[FIRST_MORTGAGE_SERVICER] ='" & Firstmortgageservicer & "'" & ",[LOAN_ID]  ='" & Loanid & "'" & ",[ZIP]  ='" & Zip & "'" & ",[LAST_NAME] ='" & Lastname & "'" & ",[REFERRAL_DATE] ='" & Referraldate & "'" & ",[REFERRAL_RESULT]  ='" & Referralresult & "'" & ",[REFERRAL_RESULT_DATE] ='" & Referralresultdate & "'" & ",[AGENCY]  ='" & Agency & "'" & ",[AGENCY_REFERRAL_ID] ='" & Agencyreferralid & "'" & ",[COUNSELOR_FIRST_NAME] ='" & Counselorfirstname & "'" & ",[COUNSELOR_LAST_NAME] ='" & Counselorlastname & "'" & ",[ACCT_PRD] ='" & Acctprd & "' Where REFERRAL_EMAIL_SEQ_ID = '" & REFERRALEMAILSEQID & "'"

        Dim sqlhelper As New Sqlhelper

        sqlhelper.ExecuteNonQuery(Sqlstr)

        Return "Email Log Updated."
    End Function
    Public Function UpdateReferralforEmail(ByVal Toemailaddr As String, ByVal Emailnote As String, ByVal ReferralSeqId As Integer)

        Dim Sqlstr As String

        If Emailnote.Length > 1000 Then
            Emailnote = Emailnote.Substring(0, 998)
        End If

        Sqlstr = "UPDATE [REFERRAL]Set  [TO_EMAIL_ADDR] ='" & Toemailaddr & "'" & " ,[EMAIL_NOTE] = '" & Emailnote & "' Where REFERRAL_SEQ_ID = '" & ReferralSeqId & "'"
    ...
    End Function
    Public Function EmailLog(ByVal Emailto, ByVal Emailfrom, ByVal Referralseqid, ByVal Source) As String
        Return EmailLogWithAttachment(Emailto, Emailfrom, Referralseqid, Source, "", "", "")
    End Function

    Private Function ReturnOnlyDate(ByVal DateValue)
        Return Month(DateValue) & "/" & Day(DateValue) & "/" & Year(DateValue)
    End Function

    Public Function GetEmailLog(ByVal StartDate As String, ByVal EndDate As String, ByVal ByValServicer As String) As SqlDataReader
        Dim Sqlstr As String

        If (Len(ByValServicer) > 0) Then

            Sqlstr = "Select SENT_DATE, EMAIL_TO, EMAIL_FROM, FIRST_MORTGAGE_SERVICER, LOAN_ID, ZIP, LAST_NAME, CONVERT(CHAR(10), REFERRAL_DATE, 101) REFERRAL_DATE, REFERRAL_RESULT, CONVERT(CHAR(10), REFERRAL_RESULT_DATE, 101) REFERRAL_RESULT_DATE, AGENCY, AGENCY_REFERRAL_ID, COUNSELOR_FIRST_NAME, COUNSELOR_LAST_NAME, ACCT_PRD, ATTACHMENT_1, ATTACHMENT_2, ATTACHMENT_3, REFERRAL_SEQ_ID, Source, REFERRAL_EMAIL_SEQ_ID, FIRST_MORTGAGE_SEQ_ID  From REFERRAL_EMAIL_SUMMARY Where SENT_DATE between '" & StartDate & "' and '" & EndDate & "' and FIRST_MORTGAGE_SEQ_ID in (" & ByValServicer & ") "
        Else
            Sqlstr = "Select SENT_DATE, EMAIL_TO, EMAIL_FROM, FIRST_MORTGAGE_SERVICER, LOAN_ID, ZIP, LAST_NAME, CONVERT(CHAR(10), REFERRAL_DATE, 101) REFERRAL_DATE, REFERRAL_RESULT, CONVERT(CHAR(10), REFERRAL_RESULT_DATE, 101) REFERRAL_RESULT_DATE, AGENCY, AGENCY_REFERRAL_ID, COUNSELOR_FIRST_NAME, COUNSELOR_LAST_NAME, ACCT_PRD, ATTACHMENT_1, ATTACHMENT_2, ATTACHMENT_3, REFERRAL_SEQ_ID, Source, REFERRAL_EMAIL_SEQ_ID, FIRST_MORTGAGE_SEQ_ID  From REFERRAL_EMAIL_SUMMARY Where SENT_DATE between '" & StartDate & "' and '" & EndDate & "'"
        End If
